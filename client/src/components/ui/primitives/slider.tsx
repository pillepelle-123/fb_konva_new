...
export function Slider({ 
  ...
  return (
    <div className={`${className} flex flex-row gap-1 w-full`}>
      <div className="flex-1 min-w-0">
        {hasLabel ? (
          <Tooltip content={label} side={tooltipPosition} fullWidth>
            {sliderInput}
          </Tooltip>
        ) : (
          sliderInput
      ...
      <span 
        className={`text-xs text-muted-foreground flex-shrink-0 inline-flex items-baseline py-1 pl-1 relative ${disabled ? '' : 'cursor-pointer hover:bg-background/50 px-1 rounded'}`}
      ...
      >
        ...
        <span className="invisible inline-flex items-baseline" aria-hidden="true">
          <span style={{ width: `${currentDisplayValue.toString().length * 0.5}rem`, minWidth: '1rem', textAlign: 'right', display: 'inline-block' }}>
            {currentDisplayValue}
          </span>
          <span style={{ width: '1rem', minWidth: '1rem', textAlign: 'left', display: 'inline-block' }}>
            {unit}
          </span>
        </span>
        {/* Actual content - positioned absolutely to overlay placeholder */}
        <span className="absolute inset-0 flex items-baseline">
          <span className="inline-block text-right" style={{ width: `${currentDisplayValue.toString().length * 0.5}rem`, minWidth: '1rem' }}>
            {isEditing ? (
              <Input
                value={inputValue}
          ...
          <span className="inline-block text-left" style={{ width: '1rem', minWidth: '1rem' }}>
            {unit}
          </span>
        </span>
      </span>
